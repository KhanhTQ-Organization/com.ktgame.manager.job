...
namespace com.ktgame.manager.job.math.geometry
{
    public struct JobMaxNormalized
    {
        ...
        {
            NativeArray<float2> result = new NativeArray<float2>(1, Allocator);
        ...
        private struct JobMaxNormalizedModel : Unity.Jobs.IJob
        {
            [ReadOnly]
            public float DisX;
            ...
            public void Execute()
            {
                float x = DisX > 0 ? DisX : -DisX;
                float y = DisY > 0 ? DisY : -DisY;
